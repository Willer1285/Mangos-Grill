    ...
    <footer className="bg-brown-900 text-cream-200">
      <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              {(brand.displayMode === "text" || brand.displayMode === "both") && (
                <span className="text-lg font-semibold text-white">{brand.brandName}</span>
              )}
            </div>
            <p className="mt-4 text-sm leading-relaxed text-cream-400">{t("description")}</p>
            <div className="mt-4 flex gap-3">
              {socialLinks.map((social) => (
                <a
                  key={social.name}
                  href={social.href}
                  className="text-cream-400 transition-colors hover:text-terracotta-400"
          ...
          {/* Explore */}
          <div>
            <h3 className="text-sm font-semibold text-white">{t("explore")}</h3>
            <ul className="mt-4 space-y-2">
              {(["home", "menu", "locations", "reservations"] as const).map((key) => (
                <li key={key}>
                  <Link
                    href={key === "home" ? "/" : `/${key}`}
                    className="text-sm text-cream-400 transition-colors hover:text-terracotta-400"
                  >
                    {tNav(key)}
                  </Link>
                </li>
        ...
        <div className="mt-10 border-t border-brown-800 pt-6">
          <div className="flex flex-col items-center justify-between gap-4 sm:flex-row">
            ...
            <div className="flex gap-6">
              <Link href="#" className="text-xs text-cream-400 hover:text-terracotta-400">
                {t("privacyPolicy")}
              </Link>
              <Link href="#" className="text-xs text-cream-400 hover:text-terracotta-400">
                {t("termsOfService")}
